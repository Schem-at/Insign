...
use crate::ast::{BooleanExpr, RegionTable, RegionEntry, SourceLocation};
...
impl<'a> EvaluationContext<'a> {
    ...
    fn evaluate_region(&mut self, region_name: &str) -> Result<Vec<BoxPair>, ParseError> {
        ...
        if self.evaluation_path.contains(&region_name.to_string()) {
            let cycle_start = self.evaluation_path.iter()
        ...
        if let Ok(boxes) = &result {
            self.cache.insert(region_name.to_string(), EvaluatedRegion {
                boxes: boxes.clone(),
                evaluated: true,
            });
        }
    ...
    fn evaluate_expression(&mut self, expr: &BooleanExpr, current_region: &str, source: &SourceLocation) -> Result<Vec<BoxPair>, ParseError> {
        match expr {
...
fn check_boxes_bounds(boxes: &[BoxPair]) -> Result<(), ParseError> {
    for (min_corner, max_corner) in boxes {
        ...
        for i in 0..3 {
            if min_corner[i] == i32::MIN || min_corner[i] == i32::MAX ||
               max_corner[i] == i32::MIN || max_corner[i] == i32::MAX {
                return Err(ParseError::Internal {
                    message: format!("Coordinate overflow detected: box ({:?}, {:?})", min_corner, max_corner),
                    position: 0,
                });
            }
        }
...
pub fn evaluate_geometry(table: &RegionTable) -> Result<BTreeMap<String, Vec<BoxPair>>, ParseError> {
...
pub fn evaluate_region_boxes(table: &RegionTable, region_name: &str) -> Result<Vec<BoxPair>, ParseError> {
    let mut context = EvaluationContext::new(table);
...
fn subtract_box(from: BoxPair, subtract: BoxPair) -> Vec<BoxPair> {
    ...
    if from_min[0] < sub_min[0] {
        result.push((
            from_min,
            [sub_min[0] - 1, from_max[1], from_max[2]]
        ));
    }
    ...
    if from_max[0] > sub_max[0] {
        result.push((
            [sub_max[0] + 1, from_min[1], from_min[2]],
            from_max
        ));
    }
    ...
    if from_min[2] < sub_min[2] {
        result.push((
            [x_min, y_min, from_min[2]],
            [x_max, y_max, sub_min[2] - 1]
        ));
    }
    ...
    if from_max[2] > sub_max[2] {
        result.push((
            [x_min, y_min, sub_max[2] + 1],
            [x_max, y_max, from_max[2]]
        ));
    }
...
mod tests {
    ...
    fn make_test_table() -> RegionTable {
        ...
        table.regions.insert("base".to_string(), RegionEntry::Accumulator {
            boxes: vec![
                ([0, 0, 0], [1, 1, 1]),
                ([2, 2, 2], [3, 3, 3]),
            ],
        ...
        table.regions.insert("ext".to_string(), RegionEntry::Accumulator {
            boxes: vec![([10, 10, 10], [11, 11, 11])],
            sources: vec![SourceLocation::new(1, 0)],
        });
        
        // Add defined region "combined" that references "base + ext"
        table.regions.insert("combined".to_string(), RegionEntry::Defined {
        ...
        table.regions.insert("__anon_0_2".to_string(), RegionEntry::Anonymous {
    ...
    fn test_self_reference_error() {
        ...
        table.regions.insert("self_ref".to_string(), RegionEntry::Defined {
    ...
    fn test_cycle_detection() {
        ...
        table.regions.insert("a".to_string(), RegionEntry::Defined {
            expr: BooleanExpr::RegionRef("b".to_string()),
            source: SourceLocation::new(0, 0),
        });
        
        table.regions.insert("b".to_string(), RegionEntry::Defined {
