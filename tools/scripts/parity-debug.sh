...
run_parity_test() {
    ...
    # Run FFI
    local ffi_exit=0
    if ! python3 tools/parity/ffi_runner.py "$input_file" > "$ffi_output" ; then
        ffi_exit=$?
    fi
    
    # Run WASM
    local wasm_exit=0
    if ! node tools/parity/wasm_runner.js "$input_file" > "$wasm_output" ; then
        wasm_exit=$?
    fi
    ...
    if ! jq -S . "$ffi_output" > "$ffi_canon" ; then
    ...
    if ! jq -S . "$wasm_output" > "$wasm_canon" ; then
    ...
    if [[ -f "$expected_file" ]]; then
        ...
        if ! jq -S . "$expected_file" > "$expected_canon" ; then
