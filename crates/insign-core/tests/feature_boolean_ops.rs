...
mod boolean_ops_enabled_tests {
    ...
    fn test_boolean_ops_enabled_minus_works() {
        ...
        assert!(result.is_ok(), "Difference operator should work with boolean_ops feature enabled");
        ...
        assert!(!boxes.is_empty(), "Difference should produce at least one bounding box");
    }
    ...
    fn test_boolean_ops_enabled_and_works() {
        ...
        let result = compile(&units);
        assert!(result.is_ok(), "Intersection operator should work with boolean_ops feature enabled");
        ...
        let result_entry = dsl_map.get("result").unwrap();
        let boxes = result_entry.bounding_boxes.as_ref().unwrap();
        assert_eq!(boxes.len(), 1, "Intersection should produce one bounding box");
        assert_eq!(boxes[0], ([2, 2, 2], [4, 4, 4]), "Intersection should be correct");
    }
    ...
    fn test_boolean_ops_enabled_xor_works() {
        ...
        let result = compile(&units);
        assert!(result.is_ok(), "XOR operator should work with boolean_ops feature enabled");
        ...
        let result_entry = dsl_map.get("result").unwrap();
        let boxes = result_entry.bounding_boxes.as_ref().unwrap();
        assert!(!boxes.is_empty(), "XOR should produce at least one bounding box");
    }
    ...
    fn test_plus_operator_still_works_with_feature() {
        ...
        let result = compile(&units);
        assert!(result.is_ok(), "Union operator should work with boolean_ops feature enabled");
