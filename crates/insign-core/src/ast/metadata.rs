...
use crate::{BoxPair, DslEntry, ParseError};
...
impl EvaluatedRegionTable {
    ...
    pub fn set_region_boxes(&mut self, region: String, boxes: Option<Vec<BoxPair>>) {
        let entry = self
            .regions
            .entry(region)
            .or_insert_with(|| EvaluatedRegionData {
                boxes: None,
    ...
    pub fn add_metadata(
        ...
        if let Some(existing) = entry.metadata.get(&key) {
            if existing.value != assignment.value {
                return Err(ParseError::MetadataConflict(Box::new(
                    crate::MetadataConflictError {
                        region,
                        key,
        ...
        match &meta_stmt.statement {
            MetadataStatement::Current { key, value } => {
                ...
                let target_region = find_last_geometry_in_tuple(geom_stmts, meta_stmt.tuple_idx)
                    .ok_or_else(|| ParseError::NoCurrentRegion {
                        source: source.clone(),
                    })?;
            ...
            MetadataStatement::Targeted { target, key, value } => {
                ...
                if is_wildcard_pattern(target) {
                    ...
                    let matching_regions: Vec<String> = evaluated_table
                        .regions
                        .keys()
...
pub fn build_dsl_map(evaluated_table: EvaluatedRegionTable) -> BTreeMap<String, DslEntry> {
    ...
    for (region_name, region_data) in evaluated_table.regions {
        ...
        let metadata: BTreeMap<String, Value> = region_data
            .metadata
            .into_iter()
...
mod tests {
    ...
    fn test_find_last_geometry_in_tuple() {
        let geom_stmts = vec![
            make_geom_stmt(
                0,
                0,
                make_rc(Some("first".to_string()), ([0, 0, 0], [1, 1, 1])),
            ),
            make_geom_stmt(
                0,
                1,
                make_rc(Some("second".to_string()), ([2, 2, 2], [3, 3, 3])),
            ),
            make_geom_stmt(
                1,
                0,
                make_rc(Some("third".to_string()), ([4, 4, 4], [5, 5, 5])),
            ),
        ];

        assert_eq!(
            find_last_geometry_in_tuple(&geom_stmts, 0),
            Some("second".to_string())
        );
        assert_eq!(
            find_last_geometry_in_tuple(&geom_stmts, 1),
    ...
    fn test_find_last_geometry_anonymous() {
        let geom_stmts = vec![
            ...
            make_geom_stmt(
                0,
                1,
                make_rc(Some("named".to_string()), ([2, 2, 2], [3, 3, 3])),
            ),
    ...
    fn test_current_region_metadata() {
        ...
        evaluated_table.set_region_boxes(
            "test_region".to_string(),
            Some(vec![([0, 0, 0], [1, 1, 1])]),
        );

        let geom_stmts = vec![make_geom_stmt(
            0,
            0,
            make_rc(Some("test_region".to_string()), ([0, 0, 0], [1, 1, 1])),
        )];

        let meta_stmts = vec![make_meta_stmt(
            0,
            1,
            MetadataStatement::Current {
                key: "label".to_string(),
    ...
    fn test_metadata_conflict_detection() {
        ...
        let geom_stmts = vec![
            ...
            make_geom_stmt(
                1,
                0,
                make_rc(Some("test".to_string()), ([2, 2, 2], [3, 3, 3])),
            ),
        ];
    ...
    fn test_wildcard_metadata() {
        ...
        // Add some regions
        evaluated_table
            .set_region_boxes("cpu.core".to_string(), Some(vec![([0, 0, 0], [1, 1, 1])]));
        evaluated_table
            .set_region_boxes("cpu.cache".to_string(), Some(vec![([2, 2, 2], [3, 3, 3])]));
        evaluated_table
            .set_region_boxes("gpu.core".to_string(), Some(vec![([4, 4, 4], [5, 5, 5])]));
        ...
        assert_eq!(
            evaluated_table.regions["cpu.*"].metadata["power"].value,
            json!("low")
        );

        // Should apply to cpu.core and cpu.cache but not gpu.core
        assert_eq!(
            evaluated_table.regions["cpu.core"].metadata["power"].value,
            json!("low")
        );
        assert_eq!(
            evaluated_table.regions["cpu.cache"].metadata["power"].value,
            json!("low")
        );
        assert!(!evaluated_table.regions["gpu.core"]
            .metadata
            .contains_key("power"));
    }
    ...
    fn test_build_dsl_map_excludes_empty_anonymous() {
        ...
        evaluated_table
            .set_region_boxes("__anon_0_0".to_string(), Some(vec![([0, 0, 0], [1, 1, 1])]));

        // Add anonymous region with metadata
        evaluated_table
            .set_region_boxes("__anon_0_1".to_string(), Some(vec![([2, 2, 2], [3, 3, 3])]));
        evaluated_table
            .add_metadata(
