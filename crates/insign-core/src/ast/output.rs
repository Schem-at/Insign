...
mod tests {
    ...
    fn make_entry(boxes: Option<Vec<([i32; 3], [i32; 3])>>, metadata: &[(&str, serde_json::Value)]) -> DslEntry {
    ...
    fn test_deterministic_ordering() {
        ...
        dsl_map.insert("zebra".to_string(), make_entry(Some(vec![([0, 0, 0], [1, 1, 1])]), &[]));
        dsl_map.insert("$global".to_string(), make_entry(None, &[("version", json!("1.0"))]));
        dsl_map.insert("cpu.*".to_string(), make_entry(None, &[("power", json!("low"))]));
        dsl_map.insert("apple".to_string(), make_entry(Some(vec![([2, 2, 2], [3, 3, 3])]), &[]));
        dsl_map.insert("*.cache".to_string(), make_entry(None, &[("size", json!(1024))]));
        dsl_map.insert("__anon_0_0".to_string(), make_entry(Some(vec![([4, 4, 4], [5, 5, 5])]), &[("label", json!("anon"))]));
    ...
    fn test_filter_anonymous_without_metadata() {
        ...
        dsl_map.insert("__anon_0_0".to_string(), make_entry(Some(vec![([0, 0, 0], [1, 1, 1])]), &[]));
        
        // Anonymous region with metadata - should be kept
        dsl_map.insert("__anon_0_1".to_string(), make_entry(Some(vec![([2, 2, 2], [3, 3, 3])]), &[("label", json!("kept"))]));
        
        // Named region without metadata - should be kept
        dsl_map.insert("named".to_string(), make_entry(Some(vec![([4, 4, 4], [5, 5, 5])]), &[]));
    ...
    fn test_shape_final_output_complete() {
        ...
        dsl_map.insert("region_z".to_string(), make_entry(Some(vec![([0, 0, 0], [1, 1, 1])]), &[]));
        dsl_map.insert("$global".to_string(), make_entry(None, &[("version", json!("1.0"))]));
        dsl_map.insert("cpu.*".to_string(), make_entry(None, &[("power", json!("low"))]));
        dsl_map.insert("region_a".to_string(), make_entry(Some(vec![([2, 2, 2], [3, 3, 3])]), &[("type", json!("test"))]));
        dsl_map.insert("__anon_0_0".to_string(), make_entry(Some(vec![([4, 4, 4], [5, 5, 5])]), &[])); // No metadata - filtered
        dsl_map.insert("__anon_0_1".to_string(), make_entry(Some(vec![([6, 6, 6], [7, 7, 7])]), &[("anon_label", json!("kept"))])); // Has metadata - kept
        dsl_map.insert("*.cache".to_string(), make_entry(None, &[("size", json!(2048))]));
