...
pub enum ParseError {
    ...
    InvalidVec3 { position: usize, message: String },

    #[error("Invalid box at position {position}: {message}")]
    InvalidBox { position: usize, message: String },

    #[error("Phase 0 supports only '+' operator at position {position}. Found '{operator}'")]
    UnsupportedOperator { position: usize, operator: String },
    ...
    FeatureGated { position: usize, operator: String },
    ...
    Internal { message: String, position: usize },
    ...
    NoCurrentRegion { source: crate::ast::SourceLocation },
}
...
mod tests {
    ...
    fn test_compile_cpu_cache_region_name() {
        ...
        let units = vec![(
            [0, 0, 0],
            "@cpu.cache=rc([0,0,0],[1,1,1])\n#doc.label=\"CPU Cache\"".to_string(),
        )];
        let result = compile(&units);
        assert!(
            result.is_ok(),
            "Should parse region names containing 'rc' or 'ac' substrings"
        );

        let dsl_map = result.unwrap();
        // The region should be present in the output
        assert!(
            dsl_map.contains_key("cpu.cache"),
            "cpu.cache region should be present in output"
        );
    }
    ...
    fn test_m8_complete_end_to_end() {
        ...
        assert!(
            dsl_map.contains_key("__anon_0_0"),
            "Should have anonymous region with metadata"
        );
        assert!(dsl_map.contains_key("cpu.*"), "Should have wildcard entry");
        assert!(
            dsl_map.contains_key("cpu.cache"),
            "Should have named region"
        );
...
mod m12_feature_gate {
    #[cfg(all(test, not(feature = "boolean_ops")))]
    use super::compile;
    ...
    fn test_boolean_ops_disabled_by_default_minus() {
        ...
        let units = vec![([0, 0, 0], "@result=a-b".to_string())];
...
mod m9_diagnostics {
    ...
    fn test_unknown_region_error() {
        ...
        let units = vec![([0, 0, 0], "@result=nonexistent+alsomissing".to_string())];
    ...
    fn test_no_current_region_error() {
        ...
        let units = vec![([0, 0, 0], "#label=\"No region to attach to\"".to_string())];
    ...
    fn test_disallowed_boolean_operators() {
        ...
        for (operator_name, input) in test_cases {
            let units = vec![([0, 0, 0], input.to_string())];
            ...
            let error_message = format!("{}", result.unwrap_err());
            assert_snapshot!(
                format!("disallowed_operator_{}", operator_name),
                error_message
            );
        }
    }
