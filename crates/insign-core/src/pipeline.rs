...
use crate::lexer::{split_statements, filter_comments};
use crate::parser::geom::GeometryParser;
use crate::parser::meta::MetadataParser;
use crate::ast::{
    GeomStmt, MetaStmt, 
    assemble_region_table, evaluate_geometry,
    EvaluatedRegionTable, apply_metadata_pass, shape_final_output
};
...
pub fn compile_pipeline(units: &[([i32; 3], String)]) -> Result<DslMap, Error> {
    ...
    let region_table = assemble_region_table(all_geom_stmts.clone(), all_meta_stmts.clone(), units)?;
...
mod tests {
    ...
    fn test_compilation_with_anonymous_and_global() {
        ...
        assert_eq!(anon_entry.metadata["label"], json!("Anonymous with metadata"));
    ...
    fn test_compilation_with_wildcards() {
        ...
        assert_eq!(dsl_map.get("cpu.core").unwrap().metadata["power"], json!("low"));
        assert_eq!(dsl_map.get("cpu.cache").unwrap().metadata["power"], json!("low"));
        
        // gpu.core should not have the power metadata
        assert!(!dsl_map.get("gpu.core").unwrap().metadata.contains_key("power"));
    }
    ...
    fn test_compilation_error_propagation() {
        ...
        let units = vec![
            ([0, 0, 0], "@test=rc([0,0,0],[1,1,1])\n#label=\"First\"".to_string()),
            ([5, 5, 5], "@test=rc([2,2,2],[3,3,3])\n#label=\"Second\"".to_string()), // Conflict!
        ];
    ...
    fn test_mixed_statements_with_comments() {
        ...
        let units = vec![
            ([0, 0, 0], "; Define a base region\n@base=rc([0,0,0],[1,1,1])\n; End base definition".to_string()),
            ([5, 5, 5], "; Another region\n@ext=rc([0,0,0],[2,2,2])\n; Extension complete".to_string()),
            ([0, 0, 0], "; Combine regions\n@combined=base+ext\n; Combined region created".to_string()),
